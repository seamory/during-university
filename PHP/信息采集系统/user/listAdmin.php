...
//$rows=fetchAll($sql);
//$rows=mysql::query('*','dm_user',"usergroup='administrators'",'mul',MYSQL_ASSOC);
$pageControl = new pageControl;
$pageControl -> setSql('*', 'dm_user', "usergroup='administrators'", $_REQUEST['query'], 10);
$rows = $pageControl -> getRows();
//$rows=getAllAdmin();

if(!$rows){
	common::alertMsg("sorry,没有管理员,请添加!","addAdmin.php");
	exit;
}
?>
...
<div class="am-scrollable-horizontal">
	...
	<table class="am-table am-table-bordered am-table-radius am-text-center">
		...
		<?php  $i=1;if($rows)foreach($rows as $row):?>
			...
			<?php endforeach;?>
			<tr>
				<td colspan="5"><?php $pageControl -> showPage('?page=43', 'query');?></td>
			</tr>
		</tbody>
	</table>
</div>
