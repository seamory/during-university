		...
		for($y = 2; $y < sizeof($TABLE); $y++){
			for($x = 2; $x < sizeof($TABLE[$y]); $x++){
				if( $TABLE[$y][$x] == '&nbsp;' ){
					...
					if(preg_match($regx[2], $TABLE[$y][$x])){
						preg_match_all($regx[2], $TABLE[$y][$x], $week, PREG_SET_ORDER);
					...
					}elseif(preg_match($regx[3], $TABLE[$y][$x])){
						preg_match_all($regx[3], $TABLE[$y][$x], $week, PREG_SET_ORDER);
						foreach($week as $key => $val){
							$TMP_num = $y - 1;
							$TMP_regx = '/' . $TMP_num . '/';
							if( preg_match($TMP_regx, $val[2]) )
					...
					}elseif(preg_match($regx[0], $TABLE[$y][$x])){
						preg_match_all($regx[0], $TABLE[$y][$x], $week, PREG_SET_ORDER);
						foreach($week as $key => $val)
						...
						preg_match_all($regx[1], $TABLE[$y][$x], $week, PREG_SET_ORDER);
						foreach($week as $key => $val)
