...
NR==FNR{
    ...
    if(match($5,"借书") && bookUnique[$1][$4]!=0){
        for(i in book[$4]){
    ...
    for(i in name) {
        for(j in person[i]){
            if(i!=j){
                if(str==""){
                    str = name[j]"("j") "person[i][j]/count[i]*100"% "bookList[i][j];
                } else {
                    str = name[j]"("j") "person[i][j]/count[i]*100"% "bookList[i][j]", "str;
                }
            }
        }
        print(name[i],i,count[i]" | "str);
        str="";
    }
...
NR==FNR{    # 通过NR与FNR来awk当前读取的文件是否为第一个文件
    if(match($5,"借书")) {  # 通过match判断当前读取的记录是否为借书记录
        ...
        bookUnique[$1][$4]=1;   # 排除一个用户多次借阅同一本书导致的借阅被重复计算
    }
} NR!=FNR { # 通过NR与FNR来awk当前读取的文件是否为第二个文件
    if(match($5,"借书") && bookUnique[$1][$4]!=0) {  # 通过match判断当前读取的记录是否为借书记录并确保此书的书籍记录是第一次访问
